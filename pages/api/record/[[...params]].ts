...
export default async function handler(request: NextApiRequest, response: NextApiResponse): Promise<void> {
  ...
  if (request.query.params === undefined) {
    if (request.method === 'DELETE' || request.method === 'PUT') {
      const recordData = await getRecord(data.id)

      if (!recordData) {
        return response.json({
          status: 404,
          data: null,
          message: `record not found from database with the given id ${data.id}`
        })
      }
    }
    ...
    if (request.method === 'GET') {
      ...
      if (record) {
        ...
        return response.json({
          status: 404,
          data: null,
          message: `Record not found with given id: record.id = ${data?.id}`
        })
      }
    }
