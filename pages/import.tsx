...
const exampleYamlFile = [
  'Riverside rally - A rwd:',
  '  - record:  ',
  '      time: "3:57.979" ',
  '      driver: Tatu ',
  '      car: ford coupe 4p',
...
function convertYamlToData(yamlDocument: any): ImportData {
  ...
  const tracks: ImportTrack[] = Object.keys(yamlDocument)
    .filter(key => key.toLocaleLowerCase() !== 'drivers')
    .map(key => {
      return {
        name: key,
        records: yamlDocument[key].map((recordObject: any): ImportTrackRecord | undefined => {
...
function convertYamlToSQL(importData: ImportData): string {
  ...
  sql += Object.keys(importData.drivers)
    .map(driver => {
  ...
  sql += Object.keys(importData.cars)
    .flatMap(driver => {
  ...
  sql += importData.tracks
    .map(track => {
      let sql = `INSERT INTO tracks (name) VALUES ('${track.name}');\n`

      sql += track.records
        .map(record => {
          const carsSql = record.car
            ? `(SELECT id FROM cars WHERE name = '${record.car.name}' AND drivers_id = ${record.driver})`
            : 'null'

          const sql = `INSERT INTO records (time, drivers_id, cars_id, tracks_id ) VALUES (${record.time}, ${record.driver}, ${carsSql},
...
export default function ImportDataView(): React.ReactElement {
  ...
  function onYamlTextAreaChanges(event: React.ChangeEvent): void {
  ...
  const onSqlTextAreaChanges = (): void => {}
    ...
    <>
      ...
      <main className="container">
        ...
        <textarea
          ...
          onChange={onYamlTextAreaChanges}
          rows={10}
        ></textarea>

        <div className="text-end mt-3 mb-3">
          <button type="button" className="btn btn-primary" onClick={onLoadData}>
            Convert to SQL
          </button>
        </div>
