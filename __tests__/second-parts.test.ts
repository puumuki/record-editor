...
describe('Convert seconds to minutes, seconds and hurdreds of second.', () => {
  ...
  it('SecondParts(200) should be 3 minutes 20 seconds', () => {    
    ...
    expect(hundredths).toEqual('03');
  });

  it('SecondParts(null) should give out 00:00:00', () => {    

    const secondParts = new SecondParts( null  );
    const {minutes, seconds, hundredths} = secondParts.parts;   
  ...
  it("toString()", () => {        
    const secondParts = new SecondParts('00:00:00');
    expect( secondParts.toString() ).toEqual('00:00:00');
  });    

  it("toString('1:1:1') -> '01:01:10' ", () => {        
    const secondParts = new SecondParts('1:1:1');
    expect( secondParts.toString() ).toEqual('01:01:10');

    const secondParts2 = new SecondParts('1:1:01');
    expect( secondParts2.toString() ).toEqual('01:01:01');
  });    

  it("validate('00:00:00') to be true", () => {            
    expect( SecondParts.validate('00:00:00') ).to.be.true;
    expect( SecondParts.validate('99:99:99') ).to.be.true;
    expect( SecondParts.validate('09:09:09') ).to.be.true;
  ...
  it("validate('00:aa:aa') to be false", () => {            
    ...
    expect( SecondParts.validate('001:00:001') ).to.be.false;
  ...
  it("parseTimeParts('00:aa:aa') to thrown an error", () => {                  
    ...
    }).to.throw('Given string "00:aa:aa" is in invalid format. Expecting mm:ss:hh formatted string. Where mm are minutes, ss are seconds and hh are hunderths of a second.');    

    expect( () => {
      new SecondParts('');
    }).to.throw('Given string "" is in invalid format. Expecting mm:ss:hh formatted string. Where mm are minutes, ss are seconds and hh are hunderths of a second.');    
  });
  ...
  it("set parts()", () => {                  
    ...
    expect( secondParts.rawvalue ).toEqual(200.02);
