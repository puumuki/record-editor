...
describe('Convert seconds to minutes, seconds and hurdreds of second.', () => {
  ...
  it("parseTimeParts('00:aa:aa') to thrown an error", () => {                  
    ...
    }).to.throw('Given string "00:aa:aa" is in invalid format. Expecting mm:ss.hhh formatted string. Where mm are minutes, ss are seconds and hhh are hunderths of a second.');    

    expect( () => {
      new SecondParts('');
    }).to.throw('Given string "" is in invalid format. Expecting mm:ss.hhh formatted string. Where mm are minutes, ss are seconds and hhh are hunderths of a second.');    
  });
