...
import { HistoryState, pushHistoryState } from './history'
...
export const fetchTracksDriversCars = createAsyncThunk('race-recorder/fetchTracksDriversCars', async () => {
  return await Promise.all([api.getDrivers(), api.getAllTracks(), api.getCars()])
})

export const fetchCars = createAsyncThunk('race-recorder/fetchCars', async () => {
  return await api.getCars()
})

export const fetchDrivers = createAsyncThunk('race-recorder/fetchDriver', async () => {
  return await api.getDrivers()
})

export const fetchTracks = createAsyncThunk('race-recorder/fetchTracks', async () => {
  return await api.getAllTracks()
})

export const createRecord = createAsyncThunk('race-recorder/createRecord', async (payload: Record, tuckApi) => {
  const response = await api.createRecord(payload)
  await tuckApi.dispatch(fetchTracksDriversCars())
  return response
})

export const updateRecord = createAsyncThunk('race-recorder/updateRecord', async (payload: Record, tuckApi) => {
  const response = await api.updateRecord(payload)
  await tuckApi.dispatch(fetchTracksDriversCars())
  return response
})

export const deleteRecord = createAsyncThunk('race-recorder/deleteRecord', async (payload: Record, tuckApi) => {
  const response = await api.deleteRecord(payload)
  await tuckApi.dispatch(fetchTracksDriversCars())
  return response
})

export const addTrack = createAsyncThunk('race-recorder/addTrack', async (payload: Track, tuckApi) => {
  const response = await api.addTrack(payload)
  await tuckApi.dispatch(fetchTracksDriversCars())
  return response
})

export const updateTrack = createAsyncThunk('race-recorder/updateTrack', async (payload: Track, tuckApi) => {
  const response = await api.updateTrack(payload)
  await tuckApi.dispatch(fetchTracksDriversCars())
  return response
})

export const updateTrackDrivers = createAsyncThunk('race-recorder/updateTrackDrivers', async (payload: Track) => {
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(fetchTracksDriversCars.fulfilled, (state, action) => {
        ...
        state.tracks = tracks?.length > 0 ? tracks : []
