...
interface RaceRecorderTableRowsProps {
  record_id?: number
  track_id?: number
  drivers: Driver[]
  tracks: Track[]
...
const RaceRecorderTableRows = (props: RaceRecorderTableRowsProps): React.ReactElement => {
  ...
  const rows = records
    ...
    .map((record, index) => {
      ...
      return (
        <tr key={`tr-record-${index}`} className={rowStyleClass}>
          <td>{new SecondParts(record.time).format}</td>
          <td>{driver?.name}</td>
          <td>{car?.name}</td>
          <td>
            <Score score={car?.scores} />
          </td>
          {isAdmin(session) && (
            <td className="text-end">
              <button
                ...
                className="btn btn-primary modify-record me-sm-2 mb-2 mb-sm-0"
              >
                {t('racerecorder.modify')}
              </button>
              <button type="button" data-record-id={record.id} className="btn btn-primary delete-record">
                {t('racerecorder.delete')}
              </button>
            </td>
          )}
        </tr>
