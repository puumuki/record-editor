...
export default function RaceRecorder(): React.ReactElement {
  ...
  useWindowEventListener('click', event => {
    ...
    if (!target.closest('.race-recorder ')) {
      batch(() => {
        void dispatch(setTime(''))
        void dispatch(setRecordId(undefined))
        void dispatch(setModifyRecordId(undefined))
        void dispatch(setDriverId(undefined))
      })
    }
  })

  useEffect(() => {
    void dispatch(fetchTracksDriversCars())
  ...
  function onTrackChange(event: React.ChangeEvent): void {
  ...
  function onCarChanges(event: React.ChangeEvent): void {
    const target = event.target as HTMLSelectElement
  ...
  function onClickModifyRecord(event: React.MouseEvent): void {
    ...
    batch(() => {
      ...
      if (state.modify_record_id) {
        void dispatch(updateRecord({ ...record, id: state.modify_record_id }))
      } else {
        void dispatch(createRecord(record))
      }
    })
  }

  function onTableClick(event: React.MouseEvent): void {
    const target = event.target as HTMLButtonElement
    if (target.dataset.recordId) {
      const recordId = parseInt(target.dataset.recordId)
      const record = state.tracks
        .find(track => track.id === state.track_id)
        ?.records.find(record => record.id === recordId)

      if (record?.drivers_id) {
  ...
  function onConfirmDelete(): void {
    ...
    if (record) {
      batch(() => {
        ...
        void dispatch(deleteRecord(record))
      })
    }
  }
  ...
  function onDriverChanges(event: React.ChangeEvent): void {
  ...
  return (
    <section className="race-recorder container">
      {state.drivers.length === 0 && (
        <WarningMessage
          title={t('racerecorder.database.connection.error.title')}
          message={t('racerecorder.database.connection.error.description')}
        ></WarningMessage>
      )}

      {state.showTrackEditorModal && (
        <TrackEditorModel
          showTrackEditorModal={state.showTrackEditorModal}
      ...
      {state.showConfirmDialog && (
        <ConfirmModal
          title={t('racerecorder.confirmdialog.deletetime.title')}
        ...
        <div className="col-lg-3 col-md-12">
          ...
          >
            {sortTrackAlphabetically(state.tracks).map(track => {
              return (
                <option key={track.id} value={track.id ? track.id : undefined}>
                  {track.name}
                </option>
              )
            })}
        ...
        {isAdmin(session) && (
          <>
            <div className="col-lg-3 col-md-12 mt-3 mt-lg-0 d-flex align-items-end">
              <button type="button" className="btn btn-primary" onClick={updateTrack}>
                {t('racerecorder.modifytrackbutton')}
              </button>
              <button type="button" className="btn btn-primary ms-2" onClick={createTrack}>
                {t('racerecorder.createtrackbutton')}
              </button>
            </div>
            ...
            <div className="col-md-12 col-lg-6 mt-3 mt-lg-0 d-flex justify-content-lg-end">
              <div className="form-group ms-3">
                ...
                >
                  ...
                  {state.drivers.map(driver => {
                    return (
                      <option value={driver.id ?? undefined} key={driver.id}>
                        {driver.name}
                      </option>
                    )
                  })}
              ...
              <div className="form-group ms-3">
                ...
                >
                  ...
                  {sortCarsAlphabetically(state.cars.filter(car => car.drivers_id === state.driver_id)).map(car => {
                    return (
                      <option value={car.id ?? undefined} key={car.id}>
                        <CarOption car={car}></CarOption>
                      </option>
                    )
                  })}
                </select>
              ...
              <div className="form-group ms-3 d-flex align-items-end me-2">
                <button
                  ...
                  onClick={onClickModifyRecord}
                >
                  {state.modify_record_id ? t('racerecorder.modify') : t('racerecorder.add')}
                </button>
              </div>
            </div>
          </>
      ...
      {state.status === 'loading' && <Spinner></Spinner>}
    </section>
  )
}
